...
var data = ['Ajit', 'Mahesh', 'Amit', 'Ajit', 'Amit']; // use reducer function to return number of times the name occured in array
...
var res = data.reduce(function (names, curname) {
  // check the repeatation of curname in names and if found
  // increase counter by 1
  if (curname in names) {
    names[curname]++;
...
console.log("Frequency = ".concat(JSON.stringify(res)));
var res1 = data.reduce(countFrequency, {});
console.log("Frequency = ".concat(JSON.stringify(res1))); // find out the result group by object
...
function printGroup(sourceArray, pname) {
  ...
  return sourceArray.reduce(function (emp, obj) {
    var groupKey = obj[pname]; // the group property
...
var groupResult = printGroup(employees, 'dname');
console.log("Result of Groups = ".concat(JSON.stringify(groupResult)));
